...
def read_pnl(filepath: str | Path) -> dict:
    ...
    for row in rows:
        ...
        if in_expenses and label and label not in ("EXPENSES ", "INCOMES", "GROSS INCOME"):
            ...
            if amount > 0:
                ...
                total_expenses_temp += amount

    result["total_expenses"] = round(total_expenses_temp, 2)
    result["net_profit"] = round(result["gross_income"] - total_expenses_temp, 2)
...
if __name__ == "__main__":
    ...
    filepath = r"data\input\End_of_May_manager_s_report.xlsx"
