...
CATEGORY_COLS = {
    ...
    "guichet":             17,
    "total_turnover":      18,  # Excluding airtime, carwash, lubricants in shop
    "global_total":        19,
}
...
def get_monthly_shop_metrics(
    ...
    global_total   = round(df["global_total"].sum(), 2) if df["global_total"].sum() > 0 else total_turnover
    trading_days   = len(df[df["total_turnover"] > 0])
    avg_daily      = round(global_total / trading_days, 2) if trading_days > 0 else 0.0

    # Best and worst trading days (by total_turnover)
    best_row  = df.loc[df["total_turnover"].idxmax()]
    worst_row = df.loc[df["total_turnover"].idxmin()]
...
if __name__ == "__main__":
    ...
    filepath = r"data\input\SHOP_MONTHLY_SALES_REPORT_FOR_APRIL_2026_T_E__Rwizi_.xlsx"
